(function( $ ) {
	var calenderTpl = '<div id="calTitle"><button class="month-mover prev"></button><h3 id="monthYear"></h3><button class="month-mover next"></button></div><div><div id="calThead"><div>D</div><div>L</div><div>M</div><div>M</div><div>J</div><div>V</div><div>S</div></div><div id="calTbody"></div></div><div id="calTFooter"><h3 id="eventTitle">No events today.</h3><a href="javascript:void(0);" id="calLink">ALL EVENTS</a></div>';
	var short_months = ["Enero", "Febrero", "Mayo", "Abril", "Mayo", "Junio", "Julio","Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"];
	var today = new Date();
    ...
    $.fn.miniEventCalendar = $.fn.MEC = function(options) {
		...
		function date_tpl(blurred, date, is_today, event){
			...
			if(!blurred){
		        ...
		        var tpl ="<button class='a-date "+cls+"' onclick='miFuncionCambiaFecha(\""+cur_year+"-"+mes+"-"+dia+"\")' data-event='"+JSON.stringify(event)+"'><span>"+date+"</span></button>";
			}
